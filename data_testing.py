...
if __name__ == '__main__':
    ...
    popt, _ = curve_fit(sinfunc, time[2: -2], bpmcalc.velocity_data[2: -2], p0=[10, 6.28, 1, 0])
    bpm = bpm_from_ang_freq(popt[1])
    print(bpm)

    plt.plot(time[1:len(time)-1], bpmcalc.velocity_data[1:len(time)-1], label='processed unsmoothed data')
    ...
    plt.plot(time[1:len(time)-1], bpmcalc.velocity_data[1:len(time)-1], label='processed smoothed data')
